...
class ErrorReporter {
  constructor() {
    this.buffer = typeof sessionStorage !== 'undefined'
      ? JSON.parse(sessionStorage.getItem(BUFFER_KEY) || '[]')
      : [];
    this.lastFlushAt = 0;
  ...
  reportApi(err) {
    this.enqueue({ severity: 'Error', kind: 'api-5xx', message: `HTTP ${err?.response?.status} ${err?.config?.url}`, stackTrace: JSON.stringify(err?.response?.data ?? null)?.slice(0, 1000), clientUrl: (typeof window !== 'undefined' ? window.location.href : ''), userAgent: navigator?.userAgent ?? '' });
  }
  reportClient(severity, msg, ctx) {
    this.enqueue({ severity, kind: 'client', message: msg, stackTrace: ctx?.stack, clientUrl: (typeof window !== 'undefined' ? window.location.href : ''), userAgent: navigator?.userAgent ?? '' });
  }
  _withinRate() {
  ...
  async flush() {
    if (!(typeof navigator === 'undefined' || navigator.onLine) || this.buffer.length === 0) return;
